...
def test_create_original_post(api_client):
    ...
    token_access = perform_login(
        api_client, email=user.email, password=data_user["password"]
    )
...
def test_updated_original_post(api_client):
    ...
    data = {"content": f"Atualização da postagem do {user.username}"}
...
def test_post_detail(api_client):
    ...
    assert (
        response.data["user_details"]["avatar"] == f"/media/{user.profile.avatar.name}"
    )
